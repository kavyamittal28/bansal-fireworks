...
function WholesaleProductCard({ p }) {
  const thumbnail = p.media?.[0]?.url

  return (
    <Link
      to={`/wholesale/products/${p.id}`}
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex flex-wrap gap-1 mb-2">
          ...
          <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-medium">WHOLESALE</span>
        </div>
        <h3 className="text-gray-900 font-semibold text-sm mb-3">{p.name}</h3>
        <div className="mt-auto">
          <p className="text-xs text-gray-400 mb-1">Wholesale Price</p>
          <div className="flex items-baseline gap-2 flex-wrap">
            <span className="text-blue-700 font-bold text-lg">₹{Number(p.wholesale_price).toLocaleString('en-IN')}</span>
            {p.price && (
              <span className="text-sm text-gray-400 line-through">₹{Number(p.price).toLocaleString('en-IN')} retail</span>
            )}
          </div>
        </div>
      </div>
...
export default function WholesalePage() {
    ...
    async function fetchAll() {
      try {
        const [productsRes, categoriesRes, brandsRes] = await Promise.all([
          fetch('/api/get-wholesale-products'),
          fetch('/api/categories'),
          fetch('/api/brands'),
        ])
        if (!productsRes.ok) throw new Error('Failed to load wholesale products')
  ...
  const sorted = [...filtered].sort((a, b) => {
    if (sortBy === 'price_asc') return a.wholesale_price - b.wholesale_price
    if (sortBy === 'price_desc') return b.wholesale_price - a.wholesale_price
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-semibold bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full">WHOLESALE</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900">Wholesale Catalog</h1>
            <p className="text-gray-500 text-sm mt-1">Exclusive wholesale pricing for bulk buyers and retailers.</p>
          </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {!loading && !fetchError && sorted.length === 0 && (
            <div className="text-center py-20 bg-white rounded-2xl border border-gray-200">
              ...
              <h3 className="text-gray-900 font-semibold text-lg mb-2">No wholesale products found</h3>
              <p className="text-gray-500 text-sm mb-6">
                {products.length === 0
                  ? 'No products have been assigned a wholesale price yet.'
          ...
          {!loading && sorted.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-10">
              {sorted.map(p => <WholesaleProductCard key={p.id} p={p} />)}
            </div>
          )}
          ...
          <div className="bg-gray-800 rounded-2xl p-5 sm:p-8 text-center">
            ...
            <h3 className="text-white font-bold text-base sm:text-lg mb-2">Ready to Place a Wholesale Order?</h3>
            <p className="text-gray-400 text-sm mb-5 sm:mb-6 max-w-lg mx-auto">
              Contact us to discuss bulk pricing, minimum order quantities, and delivery options for your business.
            </p>
            <Link
              to="/contact"
